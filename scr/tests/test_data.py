...
from deepchecks.vision.suites import train_test_validation, data_integrity # type: ignore
...
class PNGTxtDataset(Dataset):
    def __init__(self, dir):
        self.image_paths = [os.path.join(dir, f) for f in os.listdir(dir)
...
def test_data(report_dir, path_start, batch_size=64):
    train_path = os.path.join(path_start, 'data/processed/train')
    test_path  = os.path.join(path_start, 'data/processed/test')

    train_ds_torch = PNGTxtDataset(train_path)
    test_ds_torch  = PNGTxtDataset(test_path)
    ...
    label_map = create_label_map(train_path)
    ...
    result.save_as_html(
        os.path.join(path_start, report_dir, 'data_testing_report.html'),
        as_widget=False,         
        requirejs=True,           
        connected=False           
    )
...
def test_data_quality(report_dir, path_start, batch_size=64):
    ...
    os.makedirs(report_dir, exist_ok=True)
    result.save_as_html(
    os.path.join(path_start, report_dir, 'data_quality_report.html'),
    as_widget=False,          
    requirejs=True,          
    connected=False          
    )
